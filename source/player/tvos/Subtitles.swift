...
//        let contents = try! String(contentsOf: filePath, encoding: encoding)
    ...
    fileprivate func addSubtitleLabel() {
        ...
        guard let _ = subtitleLabel else {
            ...
            subtitleLabel?.backgroundColor = UIColor(hex6: 0x000000, alpha: 0.5)
                //UIColor(hexString: "#000000", alpha: 0.5) //UIColor.clear
            subtitleLabel?.textAlignment = .center
            subtitleLabel?.numberOfLines = 0
            #if os(tvOS)
                subtitleLabel?.font = UIFont.preferredFont(forTextStyle: .title3) //callout) // body
            #else
            ...
            #if os(tvOS)
            constraints = NSLayoutConstraint.constraints(withVisualFormat: "H:|-(>=500)-[l]-(>=500)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["l" : subtitleLabel!])
            #else
                constraints = NSLayoutConstraint.constraints(withVisualFormat: "H:|-(20)-[l]-(20)-|", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["l" : subtitleLabel!])
            #endif
            contentOverlayView?.addConstraints(constraints)
            
            if let subtitleLabel_ = self.subtitleLabel, let contentOverlayView_ = self.contentOverlayView {
                let xConstraint = NSLayoutConstraint(item: subtitleLabel_, attribute: .centerX, relatedBy: .equal, toItem: contentOverlayView_, attribute: .centerX, multiplier: 1, constant: 0)
                contentOverlayView?.addConstraint(xConstraint)
            }

            
            #if os(tvOS)
            constraints = NSLayoutConstraint.constraints(withVisualFormat: "V:[l]-(100)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["l" : subtitleLabel!])
            #else
    ...
    fileprivate func parseSubRip(_ payload: String) -> NSDictionary? {
        ...
        do {
            ...
            let matches = regex.matches(in: payload, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, payload.count))//payload.characters.count
            for m in matches {
                ...
                var match = regex.matches(in: group, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, group.count))//group.characters.count
                ...
                match = regex.matches(in: group, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, group.count))//group.characters.count
    ...
    fileprivate func searchSubtitles(_ time: CMTime) {
        ...
        // Adjust size
        let rect = (label.text! as NSString).boundingRect(with: CGSize(width: label.bounds.width, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : label.font!], context: nil)
